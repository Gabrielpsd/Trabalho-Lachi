...
#include "console_v1.5.h" /* cabeçalho de todas as novas funções definidas neste arquivo */
...
void setDimensaoJanela(int largura, int altura)
{
    HANDLE hStdout;
    SMALL_RECT srWindow;
    COORD atualBufferSize, novoBufferSize;
	CONSOLE_SCREEN_BUFFER_INFO csbiInfo;	
	...
	/* ajustar o buffer para a nova dimensão da janela em número de caracteres. 
	   ...
	   da janela */
	       ...
	       GetConsoleScreenBufferInfo(hStdout, &csbiInfo);
		   atualBufferSize = csbiInfo.dwSize;
        /* aumentar o tamanho do buffer no caso em que for pedido para aumentar o tamanho da janela de console */
           novoBufferSize.X = largura;
	       novoBufferSize.Y = altura;
		   if ((altura > atualBufferSize.Y) || (largura > novoBufferSize.X))
...
void IdentificarEvento(EVENTO e, int nivel_detalhe)
{
     switch(e.tipo_evento) /* identificar o tipo do evento recebido como parâmetro */
	 {
			...
			case KEY_EVENT:
			     printf("\nEvento do teclado");
				 printf("\n-------------------");
				 printf("\n-> Codigo da tecla: %d", e.teclado.codigo_tecla); 
